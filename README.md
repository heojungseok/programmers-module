...
이벤트는 다른 모듈의 처리 결과를 반환받을 필요 없이, 발생한 사실을 전달할 때 사용했습니다. 글 모듈은 글·댓글 작성 사실만 알리고 회원 모듈이 활동점수를 변경하도록 했습니다. 점수 정책을 회원 모듈에서 관리하므로 정책이 바뀌어도 글 모듈을 수정할 필요가 없습니다.

보안 팁은 글 작성 결과 메시지를 완성하는 데 필요한 값이므로 HTTP API로 요청하고 응답을 받았습니다. 회원 모듈의 Facade를 직접 호출하는 대신 API를 사용해 내부 코드에 대한 의존을 줄이고, 향후 서버 분리를 고려한 통신 경계를 만들었습니다.
